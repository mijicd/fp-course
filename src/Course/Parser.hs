...
(|||) (P pa) (P pb) = P (\input -> let r = pa input in if isErrorResult r then pb input else r)
...
instance Monad Parser where
  ...
  (=<<) f (P g) = P (\input -> onResult (g input) (\input' a -> parse (f a) input'))
...
phoneParser = lift3 (\a b _ -> a :. b) digit phoneBodyParser (list $ is '#')

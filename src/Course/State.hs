...
-- (Empty,8)
findM :: Applicative f => (a -> f Bool) -> List a -> f (Optional a)
findM p = foldRight (\a -> lift2 (\b oa -> if b then Full a else oa) (p a)) (pure Empty)
...
isHappy n = 
  let square  = join (*)
      sum' n' = toInteger $ sum $ square . digitToInt <$> show' n' 
      xs      = produce sum' n
